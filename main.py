...
def generate_and_send_report():
    ...
    for q in queries:
        try:
            res = tavily.search(query=q, search_depth="advanced", topic="news", max_results=3, days=1)
    ...
    if not combined_results:
        ...
        for q in queries:
             try:
                res = tavily.search(query=q, search_depth="advanced", topic="news", max_results=3, days=2)
                combined_results.extend(res['results'])
             except: pass
    ...
    prompt = f"""
你現在是**華爾街頂級科技情報局的總編輯**。
請從以下 Tavily 搜尋回來的資料庫中，篩選出 **3 則今天最新、最具市場驅動力的「具體事件」**。

🚨 【嚴格篩選準則 - 違反將扣分】：
1. 拒絕舊聞與宏觀回顧：如果新聞是在談論「2025年的趨勢」、「長期的市場預測」等沒有具體時間點的空泛文章，請直接淘汰。
2. 顆粒度要求：我只要「具體的單一事件」（例如：哪家公司今天發布了什麼模型、哪筆收購案今天確認、哪個高管今天離職）。
3. 絕不重複：如果你發現兩篇資料在講同一件事（例如都是台積電營收），請合併成一則，不要佔用兩個名額。
